...
for i = 1:length(imfiles)
    ...
    for i = 1:length(imtime)
        fprintf('Extracting %d/%d: %s\n', i, length(imtime), awsdata.imtime(i));
        awsdata.airtemp(i) = griddata(X, Y, imdata(:,:,i), aws.lon, aws.lat, 'linear');
    end
    % Save the data
    if i == 1
        writetable(awsdata, fullfile(imfolder, 'aws_airtemp_era5land.csv'), ...
