# %% [markdown]
# Script supplied by Shunan Feng
...
# %%
#%%
carra_dir = "/home/jovyan/work/AVOCA/GEM/Development/Data/CARRA/CARRA2_cloud"
mask_path = "/home/jovyan/work/AVOCA/GEM/Development/Data/Masks/Greenlandmask.tif"
output_dir = "/home/jovyan/work/AVOCA/GEM/Development/Data/CARRA/TCC_monthly"
...
imfiles = sorted(glob.glob(f"{carra_dir}/carra2_*.nc"))
...
def process_single_file(filepath):
    ...
    try:
        with xr.open_dataset(filepath) as ds:
            tcc_var = ds["tcc"].values
            tcc_time = ds["time"]
 
            # use np.mean on tcc_var across the time dimension to get monthly mean
            with np.errstate(all="ignore"):
                monthly_mean_source = np.nanmean(tcc_var, axis=0).astype(np.float32)
 
            lon2d, lat2d = get_source_geoloc(ds)
            reprojected = np.full(mask_shape, np.nan, dtype=np.float32)
 
            reproject(
                source=monthly_mean_source,
            ...
            month_label = np.datetime_as_string(np.datetime64(tcc_time.values[0]).astype("datetime64[M]")).replace("-", "_")
            out_path = os.path.join(output_dir, f"CARRA_TCC_{month_label}.tif")
            ...
            with rio.open(
                ...
                dst.set_band_description(1, "total_cloud_cover")
